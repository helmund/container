      ...
      <div class="border-solid border p-2 pb-4 font-sans font-thin text-sm rounded shadow hover:shadow-md h-full card">
          <div class="-mx-2 -mt-2 mb-4 hidden md:block">
            <a :href="`https://www.google.com/maps/dir/?api=1&destination=${item.street}+${item.street_number}+${item.postcode}&travelmode=walking`" 
              ...
              <img :src="`https://maps.googleapis.com/maps/api/staticmap?center=${item.street}+${item.street_number}+${item.postcode}+${item.disctrict}&zoom=14&size=400x150&maptype=roadmap&markers=color:red%7C%7C${item.street}+${item.street_number}+${item.postcode}+${item.disctrict}&key=AIzaSyBQhWVUEmKhoSWt6jgazOm_NhaL84WX78g`"
